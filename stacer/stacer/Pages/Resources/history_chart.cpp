...
void HistoryChart::init()
{
    ...
    connect(SignalMapper::ins(), &SignalMapper::sigChangedAppTheme, [=] {
        ...
        QList<QAbstractAxis*> axesX = mChart->axes(Qt::Horizontal);
        if (!axesX.isEmpty()) {
            axesX.first()->setLabelsColor(chartLabelColor);
            axesX.first()->setGridLineColor(chartGridColor);
        }

        QList<QAbstractAxis*> axesY = mChart->axes(Qt::Vertical);
        if (!axesY.isEmpty()) {
            axesY.first()->setLabelsColor(chartLabelColor);
            axesY.first()->setGridLineColor(chartGridColor);
        }
